...
Mesh* MeshUtils::LoadObj(const char* filename){
    ...
    std::vector<GLuint> indices;
    //Use an map to remove duplicate vertices
    ...
    for (const auto& shape : shapes) {
        for (const auto& index : shape.mesh.indices) {
            ...
            if(it == vertex_map.end()){
                //if vertex is new, add it to the vertices and map
                GLuint new_vertex_index = (GLuint)vertices.size();
                vertex_map[vertex] = new_vertex_index;
                vertices.push_back(vertex);
                //then add the index of the new vertex to the indices
                indices.push_back(new_vertex_index);
            } else {
                //if vertex already exists, add its index to the indices
