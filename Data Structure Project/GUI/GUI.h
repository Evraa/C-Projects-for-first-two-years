...
private:
	...
	static const int	
		WindWidth=1300, WindHeight=650,		//Window width and height
	...
	void DrawPavedArea (const Tower* T);   //function to change the color of the pavedArea
public:
	GUI();
	~GUI();

	// Input Functions  ---------------------------
	void GetPointClicked(Point &P, int z=1) const; // waits a user click from the user
	string GetString() const ; // reads a string (keyboard input) from the user

	// Output Functions  ---------------------------
	void PrintMessage(string msg , int Length=0 ,int Width=5) const; // prints a message in the status bar
	...
	MenuItem getUserAction(MenuItem test = Battles_Area);
	void DrawPavedAreas (Tower* towerlist); //to draw the paved Area with another color
	void Defeat();   //draw defeat logo
	void Victory();  //draw victory logo
};
